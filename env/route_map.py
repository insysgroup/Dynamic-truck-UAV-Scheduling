...
class RouteMap:
    def __init__(self, item):
        ...
        if nodenum is None:
            self.node_num = self.data.customerNum + 1  # ����depot
        else:
        ...
        self.X = self.data.cor_X[:self.node_num]  # ��ͼ��
        self.Y = self.data.cor_Y[:self.node_num]  # ��ͼ��
        self.v_k = self.data.vehicle_speed  # �����ٶ� m/s
        self.v_u = self.data.uav_speed  # ���˻��ٶ�
        self.fli_range = self.data.uav_range * 1000 / self.v_u
        self.demand = self.data.demand[:self.node_num]  # ���������������
        self.service_time = self.data.serviceTime[:self.node_num]  # ����ʱ��
        self.require_time = self.data.readyTime[:self.node_num]  # ��������readyTime����Ϊ�ͷ���ʼʱ��
        # self.disMatrix = self.data.disMatrix[:self.node_num][:self.node_num]  # ��λ m
