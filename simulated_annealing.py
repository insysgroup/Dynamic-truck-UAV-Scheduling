...
class SA:
    ...
    def get_random_neighbor(self, sol: Solution, seed=0, truck_node=3, uav_node=1):
    ...
    def search(self):
        # print(self.route_map.require_time)
        ...
        k_iter = 1
        temp = 1500
        while iter < self.epoch_num:
            i = 0
            print(f'{iter} epoch: temperature {temp}')
            while i < self.iteration_num:
                # if iter == 5 and i == 32:
                #     a=10
                neighbors, add_uav = self.get_random_neighbor(self.current_s, seed=iter*self.iteration_num + i,
                                                              truck_node=3, uav_node=1)
                while len(neighbors) < 1:
                    neighbors, add_uav = self.get_random_neighbor(self.current_s, seed=random.randint(1, 10000),
                                                                  truck_node=3, uav_node=1)
                random_move = random.choice(neighbors)
                tem_s = self.carry_move(random_move, self.current_s)
                if tem_s is not None:
                    if tem_s.total_cost < self.current_s.total_cost:
                        ...
                        delta_cost = self.current_s.total_cost - tem_s.total_cost
                        probility = math.exp(delta_cost / temp)
                        if random.random() < probility:
                            self.current_s = tem_s
                print(f'{i} interation: cur_cost = {self.current_s.total_cost:.5f},best_cost = {self.best_cost:.5f}, no_better = {no_better}')
                df1.loc[len(df1.index)] = [len(instance.demand) - 1, iter * 50 + i, self.best_cost, time.time() - t_start]
                i += 1
                no_better += 1
            iter += 1
            temp = temp * self.decrease_rate
            print()
            if no_better > 200:
                break
        df1.to_csv('SA-动态飞行伙伴-需求-100-迭代数据.csv')
        return self.best_s
...
if __name__ == "__main__":
    ...
    for item in range(1, 11):
        ...
        tabu_algrothim = SA(instance, initial_sol)
        final_sol = tabu_algrothim.search()
        final_sol.print_solution()
        t_end = time.time()
        t_continue = t_end - t_start
        df.loc[len(df.index)] = [len(instance.demand) - 1, final_sol.total_cost, math.floor(final_sol.total_cost/10000), t_continue]
    df.to_csv('SA-动态飞行伙伴-需求-150.csv')
