...
from env.solution_ptu import Solution
import pandas as pd
import math
...
class PTU:
    ...
    def search(self):
        ...
        for i in range(self.epoch_num):
            neighbors, add_uav = self.get_random_neighbor(self.current_s, seed=i, truck_node=10, uav_node=10)
            if len(neighbors) < 1:
                continue
            if add_uav:
            ...
            no_better += 1
            print(f'{i} iteration: cur_cost = {self.current_s.total_cost:.5f},best_cost = {self.best_cost:.5f}, no_better = {no_better}',
...
if __name__ == "__main__":
    ...
    for item in range(1, 11):
        ...
        tabu_ptu = PTU(instance, initial_sol)
        final_sol = tabu_ptu.search()
        t_end = time.time()
