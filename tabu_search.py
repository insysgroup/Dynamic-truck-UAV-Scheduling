...
class TabuSearch:
    ...
    def search(self):
        ...
        for i in range(self.epoch_num):
            ...
            df1.loc[len(df1.index)] = [len(instance.demand) - 1, i, self.best_cost, time.time()-t_start]
            if no_better > 200:
                break

        df1.to_csv('idle-ITS-75-迭代数据.csv')
...
if __name__ == "__main__":
    ...
    for item in range(1, 11):
        ...
        tabu_algrothim = TabuSearch(instance, initial_sol)
        final_sol, dummy_span = tabu_algrothim.search()
        t_end = time.time()
        t_continue = t_end - t_start
        truck_time = final_sol.truck_time.item()
        idle_time = truck_time - dummy_span
        ...
        print(item, t_continue)
    df.to_csv('idle-ITS-100.csv')
